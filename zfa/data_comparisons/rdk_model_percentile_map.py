...
def perform_cv(
    ...
    print(type(module_data), type(target_resp))
    print(module_data.shape, target_resp.shape)
    print(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--transition", type=str, default="passive")
    ...
    parser.add_argument("--source_cell_type", type=str, default="core")
    parser.add_argument("--target_cell_type", type=str, default="glial")
    parser.add_argument("--source_animal", type=str, default="rnd")  # source animal ID
    parser.add_argument("--target_animal", type=int, default=0)  # target animal ID
    parser.add_argument("--job_ID", type=int, default=1)

    args = parser.parse_args()

    model_type = args.source_animal
    transition_type = args.transition
    layer = args.source_cell_type
    glial_brain_data, neural_brain_data = load_data_tensors(transition_type)
    model_data = load_model_tensors(transition_type, model_type)
    module_data = model_data[layer]
    ...
    # save the data
    save_path = os.path.join(BRAIN_MODEL_RESULTS_DIR, model_type, transition_type, save_name)
