...
#         super().__init__(world_model)
#         self.name = "3m_progress" # model-memory-mismatch
...
#         with torch.no_grad():
#             world_model_pred = self.world_model(obs, action)
#             memory_pred = self.memory(obs, action)
#             memory_loss, model_loss = self.calculate_loss(memory_pred, world_model_pred, obs_next)
#             # bias = torch.norm(world_model_pred - memory_pred, p=2, dim=-1)
#             bias = self.loss_fn(world_model_pred, memory_pred).mean(dim=-1)
#             self._reward_filter(bias)
#             r = torch.abs(bias - self.ewa_bias)
#             # r = self.huber_loss(bias, self.ewa_bias).mean(dim=-1)
#             # alt: bias = torch.abs(memory_loss - model_loss)
#             # r = (self.ewa_bias - bias).mean(dim=-1)
#             # r = (bias - self.ewa_bias).mean(dim=-1)
#             # r = -bias.mean(dim=-1)
#             # r = -self.ewa_bias.mean(dim=-1)
#             return r, self.ewa_bias, memory_loss, model_loss
#             # self._reward_filter(memory_loss)
#             # r = torch.abs(self.ewa_bias - memory_loss)
#             # return r, self.ewa_bias, memory_loss, model_loss
...
class ModelMemoryMismatch(LearningProgress):
    """
    Scalar-baseline version **with 0-action counterfactual correction**.
    • bias  = ‖(f_cur(a)−f_cur(0)) − (f_mem(a)−f_mem(0))‖²   (mean over features)
    • reward = |bias − EMA(bias)|          (absolute-value formulation)
    Action penalty, if any, is applied outside this class.
    """
...
class BeliefProgress(LearningProgress):
    """
    Dual-EMA version (minimal patch):
      • slow EMA  (gamma)  = long-horizon accumulator   m_slow
      • fast EMA  (beta)   = stripe-phase tracker       m_fast
      • reward    = |m_fast − m_slow|      (keeps your original “abs” shape)
    """
