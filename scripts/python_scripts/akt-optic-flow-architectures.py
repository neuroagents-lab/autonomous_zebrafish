...
class OpticFlowDecoder(nn.Module):
    ...
    def forward(self, x):
        ...
        # Scale
        scale_x = flow.size(3) / self.input_size[1]
        scale_y = flow.size(2) / self.input_size[0]
...
class Net_3_layers(nn.Module):
    def __init__(self, input_size):
        ...
        # OpticFlowDecoder
        self.optic_flow_decoder = OpticFlowDecoder(in_channels=256, input_size=input_size)
    ...
    def forward(self, x):
        ...
        x += residual
        x = self.relu(x)
        print(f"After third residual block: {x.shape}")
